...
const MuscleBodyMap = ({ muscleData = {} }) => {
  ...
  const colors = useMemo(() => {
    ...
    for (let i = 1; i <= Math.max(maxCount, 1); i++) {
      const intensity = 0.10 + 0.90 * ((i - 1) / Math.max(maxCount - 1, 1));
  ...
  return (
    <div style={{ display: 'flex', gap: '4px', alignItems: 'flex-start', justifyContent: 'center', overflow: 'hidden' }}>
      <div style={{ flex: 1, textAlign: 'center' }}>
        ...
        />
        <div style={{ fontSize: '9px', color: 'rgba(255,255,255,0.3)', marginTop: '-8px', letterSpacing: '1px' }}>FRONT</div>
      </div>
      <div style={{ flex: 1, textAlign: 'center' }}>
        <Body
          data={data}
          ...
          defaultStrokeWidth={0.5}
        />
        <div style={{ fontSize: '9px', color: 'rgba(255,255,255,0.3)', marginTop: '-8px', letterSpacing: '1px' }}>BACK</div>
      </div>
    </div>
  );
};
